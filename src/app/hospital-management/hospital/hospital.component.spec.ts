...
describe('HospitalComponent', () => {
  ...
  it('should initialize form and fetch hospital data on component initialization', () => {
    const spy = spyOn(component, 'loadhospitalPage');
    component.ngOnInit();
    expect(component.hospitalList.get('searchQuery').value).toEqual(null);
    expect(component.hospitalList.get('status').value).toEqual(null);
    expect(spy).toHaveBeenCalled();
  });
  it('should update data source on page change', () => {
    const spy = spyOn(component.dataSource, 'loadhospital');
    component.paginator.pageIndex = 1;
    component.loadhospitalPage();
    expect(spy).toHaveBeenCalledWith(1, 10, 'createdAt', 'desc');
  });
 
  it('should open add hospital dialog', () => {
    const spy = spyOn(component.hospitalModalPopup, 'open').and.returnValue(
      {
        afterClosed: () => of(null),
      } as any
    );
