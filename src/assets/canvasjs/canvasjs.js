/**
* @preserve CanvasJS HTML5 & JavaScript Charts - v3.6.7 GA - https://canvasjs.com/ 
...
function getFontHeightInPixels(fontFamily, fontSize, fontWeight) {
	...
	if (isNaN(height)) {
		try {
			...
			setCSSProperties(textMeasureEl, { "position": "absolute", "left": "0px", "top": "-20000px", "padding": "0px", "margin": "0px", "border": "none", "whiteSpace": "pre", "lineHeight": "normal", "fontFamily": fontFamily, "fontSize": fontSize + "px", "fontWeight": fontWeight });
...
var releaseCanvas = function (canvas) {
	canvas.width = 1;
	canvas.height = 1;
...
function addExportMenu(chart) {
	...
	if (chart.exportEnabled && chart._dropdownMenu) {
		...
		setCSSProperties(chart._dropdownMenu, { "position": "absolute", "zIndex": 1, "userSelect": "none", "MozUserSeelct": "none", "WebkitUserSelect": "none", "msUserSelect": "none", "cursor": "pointer", "right": "0px", "top": "25px", "minWidth": "120px", "outline": 0, "fontSize": "14px", "fontFamily": "Arial, Helvetica, sans-serif", "padding": "5px 0px 5px 0px", "textAlign": "left", "lineHeight": "10px", "backgroundColor": chart.toolbar.itemBackgroundColor, "boxShadow": "2px 2px 10px" + shadowColor });
		...
		setCSSProperties(exportOption, { "padding": "12px 8px 12px 8px" });
...
function addCreditLink(chart) {
	...
	if (!chart._creditLink) {
		...
		setCSSProperties(chart._creditLink, { "outline": "none", "margin": "0px", "position": "absolute", "right": "2px", "top": (chart.height - 14) + "px", "color": "dimgrey", "textDecoration": "none", "fontSize": "11px", "fontFamily": "Calibri, Lucida Grande, Lucida Sans Unicode, Arial, sans-serif"});
...
function setButtonState(chart, button, state) {
	...
	if (button.getAttribute("state") !== state) {
		...
		button.innerHTML = "<img src='" + base64Images[state].image + "' alt='" + chart._cultureInfo[state + "Text"] + "' />";
		setCSSProperties(button.childNodes[0], { "height": "95%", "pointerEvents": "none" });
	}
}
...
var CanvasJSChart = CanvasJS.Chart = (function () {
	...
	function Chart(containerId, options) {
		...
		setCSSProperties(this._toolBar, { "position": "absolute", "right": "1px", "top": "1px" });
	...
	Chart.prototype.destroy = function () {
		...
		if (this.overlaidCanvas)
			releaseCanvas(this.overlaidCanvas);

		if (this._preRenderCanvas)
			releaseCanvas(this._preRenderCanvas);

		if (this._breaksCanvas)
			releaseCanvas(this._breaksCanvas);

		if (this._eventManager && this._eventManager.ghostCanvas)
			releaseCanvas(this._eventManager.ghostCanvas);
	...
	Chart.prototype.renderElements = function () {
		...
		for (var i = 0; i < this.plotInfo.plotTypes.length; i++) {
			...
			for (var j = 0; j < plotType.plotUnits.length; j++) {
				...
				if (plotUnit.targetCanvas)
					releaseCanvas(plotUnit.targetCanvas);
	...
	Chart.prototype._plotAreaMouseUp = function (x, y) {
		...
		if (this.plotInfo.axisPlacement !== "none") {
			...
			if (this.axisX && this.axisX.length > 0) {
				for (var k = 0; k < this.axisX.length; k++) {
					if (this.axisX[k].crosshair && this.axisX[k].crosshair.enabled) {
						this.axisX[k].crosshair.renderLabel();
					}
				}
			}
			if (this.axisX2 && this.axisX2.length > 0) {
				for (var k = 0; k < this.axisX2.length; k++) {
					if (this.axisX2[k].crosshair && this.axisX2[k].crosshair.enabled) {
						this.axisX2[k].crosshair.renderLabel();
					}
				}
			}
			if (this.axisY && this.axisY.length > 0) {
				for (var k = 0; k < this.axisY.length; k++) {
					if (this.axisY[k].crosshair && this.axisY[k].crosshair.enabled) {
						this.axisY[k].crosshair.renderLabel();
					}
				}
			}
			if (this.axisY2 && this.axisY2.length > 0) {
				for (var k = 0; k < this.axisY2.length; k++) {
					if (this.axisY2[k].crosshair && this.axisY2[k].crosshair.enabled) {
						this.axisY2[k].crosshair.renderLabel();
					}
				}
			}
		}
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
			...
			if (dataPoints.length > 0) {
				...
				for (i = 0; i < dataPoints.length; i++) {
					...
					drawRect(ctx, plotUnit.axisX.reversed ? x2 : x1, y1, plotUnit.axisX.reversed ? x1 : x2, y2, color, 0, null, bevelEnabled && (plotUnit.axisY.reversed ? dataPoints[i].y < 0 : dataPoints[i].y >= 0), (plotUnit.axisY.reversed ? dataPoints[i].y >= 0 : dataPoints[i].y < 0) && bevelEnabled, false, false, dataSeries.fillOpacity);
					...
					if (isCanvasSupported)
						drawRect(this._eventManager.ghostCtx, plotUnit.axisX.reversed ? x2 : x1, y1, plotUnit.axisX.reversed ? x1 : x2, y2, color, 0, null, false, false, false, false);
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
			...
			if (dataPoints.length > 0) {
				...
				for (i = 0; i < dataPoints.length; i++) {
					...
					drawRect(ctx, x1, plotUnit.axisY.reversed ? y2 : y1, x2, plotUnit.axisY.reversed ? y1 : y2, color, 0, null, bevelEnabled && (plotUnit.axisY.reversed ? dataPoints[i].y < 0 : dataPoints[i].y >= 0), (plotUnit.axisY.reversed ? dataPoints[i].y >= 0 : dataPoints[i].y < 0) && bevelEnabled, false, false, dataSeries.fillOpacity);
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
			...
			if (dataPoints.length > 0) {
				...
				for (i = 0; i < dataPoints.length; i++) {
					...
					color = dataPoints[i].color ? dataPoints[i].color : dataSeries._colorSet[i % dataSeries._colorSet.length];
					//color = "#1B4962";
					drawRect(ctx, plotUnit.axisY.reversed ? x2 : x1, plotUnit.axisX.reversed ? y2 : y1, plotUnit.axisY.reversed ? x1 : x2, plotUnit.axisX.reversed ? y1 : y2, color, 0, null, bevelEnabled, false, false, false, dataSeries.fillOpacity);
					...
					if (isCanvasSupported)
						drawRect(this._eventManager.ghostCtx, x1, plotUnit.axisX.reversed ? y2 : y1, x2, plotUnit.axisX.reversed ? y1 : y2, color, 0, null, false, false, false, false);
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
			...
			if (dataPoints.length > 0) {
				...
				for (i = 0; i < dataPoints.length; i++) {
					...
					color = dataPoints[i].color ? dataPoints[i].color : dataSeries._colorSet[i % dataSeries._colorSet.length];
					drawRect(ctx, plotUnit.axisY.reversed ? x2 : x1, y1, plotUnit.axisY.reversed ? x1 : x2, y2, color, 0, null, bevelEnabled, false, false, false, dataSeries.fillOpacity);
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
			...
			if (dataPoints.length > 0) {
				...
				for (i = 0; i < dataPoints.length; i++) {
					...
					drawRect(ctx, plotUnit.axisX.reversed ? x2 : x1, y1, plotUnit.axisX.reversed ? x1 : x2, y2, color, borderThickness, color, bevelEnabled, bevelEnabled, false, false, dataSeries.fillOpacity);
					color = intToHexColorString(id);
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
			...
			if (dataPoints.length > 0) {
				...
				for (i = 0; i < dataPoints.length; i++) {
					...
					color = dataPoints[i].color ? dataPoints[i].color : dataSeries._colorSet[i % dataSeries._colorSet.length];
					//color = "#1B4962";
					drawRect(ctx, x1, plotUnit.axisX.reversed ? y2 : y1, x2, plotUnit.axisX.reversed ? y1 : y2, color, 0, null, bevelEnabled, false, false, false, dataSeries.fillOpacity);
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
			...
			if (dataPoints.length > 0) {
				...
				for (i = 0; i < dataPoints.length; i++) {
					...
					drawRect(ctx, plotUnit.axisX.reversed ? x2 : x1, plotUnit.axisY.reversed ? y2 : y1, plotUnit.axisX.reversed ? x1 : x2, plotUnit.axisY.reversed ? y1 : y2, barColor, borderThickness, barColor, bevelEnabled, bevelEnabled, false, false, dataSeries.fillOpacity);
					color = intToHexColorString(id);
	...
	Chart.prototype.print = function () {
		...
		setCSSProperties(printFrame, { "position": "absolute", "width": "100%", "border": "0px", "margin": "0px 0px 0px 0px", "padding": "0px 0px 0px 0px" });
		...
		printDoc.document.write('<!DOCTYPE HTML>\n<html><body><img src="' + chartImageData + '"/><body/></html>');
		if (printDoc.document.body && printDoc.document.body.style) {
			printDoc.document.body.style.margin = "0px 0px 0px 0px";
			printDoc.document.body.style.padding = "0px 0px 0px 0px";
		}
		...
		for (i = 0; i < this._stripLineLabels.length; i++) {
			...
			if (label.stripLine.labelPlacement === "outside") {
				...
				if (this._position === "bottom") {
					...
					if (this.labelAngle === 0) {
						xy.x -= label.textBlock.width / 2;
						xy.y += this.tickLength + label.textBlock.fontSize / 2 + 5;
					...
					if (this.labelAngle === 0) {
						xy.x -= label.textBlock.width / 2;
						xy.y -= (this.tickLength + label.textBlock.height - label.textBlock.fontSize / 2);
					} else {
			...
			if (this.chart.plotInfo.axisPlacement === "xySwapped") {
				...
				if (this.parent._position === "bottom" || this.parent._position === "top") {
					...
					if (this.parent.dataSeries[index].type === "rangeBar" || this.parent.dataSeries[index].type === "error") {
						...
						textBlock.text = this.labelFormatter ? this.labelFormatter({ chart: this.chart, axis: this.parent.options, crosshair: this.options, value: closest.dataPoint.y }) : !isNullOrUndefined(this.options.label) ? this.label : numberFormat(!isNullOrUndefined(label) ? label : closest.dataPoint.y, this.valueFormatString, this.chart._cultureInfo);
					}
						...
						this.value = yPercent;
						textBlock.text = this.labelFormatter ? this.labelFormatter({ chart: this.chart, axis: this.parent.options, crosshair: this.options, value: yPercent }) : !isNullOrUndefined(this.options.label) ? this.label : numberFormat(!isNullOrUndefined(label) ? label : yPercent, this.valueFormatString, this.chart._cultureInfo);
					}
					else {
				...
				if (this.parent._position === "bottom" || this.parent._position === "top") {
					...
					if (this.parent.dataSeries[index].type === "rangeColumn" || this.parent.dataSeries[index].type === "rangeArea" || this.parent.dataSeries[index].type === "error" || this.parent.dataSeries[index].type === "rangeSplineArea"
						...
						textBlock.text = this.labelFormatter ? this.labelFormatter({ chart: this.chart, axis: this.parent.options, crosshair: this.options, value: !isNullOrUndefined(label) ? label : closest.dataPoint.y }) : !isNullOrUndefined(this.options.label) ? this.label : numberFormat(!isNullOrUndefined(label) ? label : closest.dataPoint.y, this.valueFormatString, this.chart._cultureInfo);
						this.value = closest.dataPoint.y;
					}
					...
					if (hasAllLabels) {
						if (this.parent.type === "axisX") {
							...
							for (var i = 0; i < this.parent.dataSeries.length; i++) {
								...
								if (entry && entry.index >= 0) {
									textBlock.text = this.labelFormatter ? this.labelFormatter({ chart: this.chart, axis: this.parent.options, crosshair: this.options, value: !isNullOrUndefined(label) ? label : this.parent.convertPixelToValue(mouseX) }) : !isNullOrUndefined(this.options.label) ? this.label : entry.dataPoint.label;
								}
							}
						}
					}
					...
					if (hasAllLabels) {
						if (this.parent.type === "axisX") {
							...
							for (var i = 0; i < this.parent.dataSeries.length; i++) {
								...
								if (entry && entry.index >= 0) {
									textBlock.text = this.labelFormatter ? this.labelFormatter({ chart: this.chart, axis: this.parent.options, crosshair: this.options, value: !isNullOrUndefined(label) ? label : this.parent.convertPixelToValue(mouseX) }) : !isNullOrUndefined(this.options.label) ? this.label : (!isNullOrUndefined(label) ? label : entry.dataPoint.label);
								}
							}
						}
					}
	...
	Crosshair.prototype.renderLabel = function () {
		if (!isNullOrUndefined(this._textBlock) && !isNullOrUndefined(this._textBlock.text) && (typeof this._textBlock.text.valueOf() === "number" || this._textBlock.text.length > 0) && !this._hidden)
			this._textBlock.render(true);
	}
	//#endregion Crosshair
	...
	ToolTip.prototype._initialize = function () {
		...
		if (this.enabled) {
			...
			toolTipHtml.style.width = "auto";
			toolTipHtml.style.height = "auto";
			toolTipHtml.style.minWidth = "50px";
			toolTipHtml.style.lineHeight = "normal";
			toolTipHtml.style.margin = "0px 0px 0px 0px";
			toolTipHtml.style.padding = "5px";
			toolTipHtml.style.fontFamily = "Calibri, Arial, Georgia, serif";
			toolTipHtml.style.fontWeight = "normal";
			toolTipHtml.style.fontStyle = (isCanvasSupported ? "italic" : "normal");
			toolTipHtml.style.fontSize = "14px";
			toolTipHtml.style.color = "#000000";
			toolTipHtml.style.textShadow = "1px 1px 1px rgba(0, 0, 0, 0.1)";
			toolTipHtml.style.textAlign = "left";
			toolTipHtml.style.border = "2px solid gray";

			//Older browsers like IE8- don't support alpha values
			toolTipHtml.style.background = isCanvasSupported ? "rgba(255,255,255,.9)" : "rgb(255,255,255)";

			toolTipHtml.style.textIndent = "0px";
			toolTipHtml.style.whiteSpace = "nowrap";
			//toolTipHtml += "pointer-events:none;";
			toolTipHtml.style.borderRadius = "5px";

			//Disable Text Selection
			toolTipHtml.style.MozUserSelect = "none";
			toolTipHtml.style.WebkitUserSelect = "none";
			toolTipHtml.style.msUserSelect = "none";
			toolTipHtml.style.userSelect = "none";
			...
			if (!isCanvasSupported) {
				...
				/* For IE 5.5 - 7 */
				toolTipHtml.style.filter = "alpha(opacity = 90)";
				toolTipHtml.style.filter = "progid:DXImageTransform.Microsoft.Shadow(Strength=3, Direction=135, Color='#666666')";
			}

			toolTipHtml.innerText = "Sample Tooltip";

			this.container.appendChild(toolTipHtml);
		...
		if (entries.length > 0) {
			...
			if (this.enabled) {
				...
				if (toolTipInnerHtml !== null) {
					...
					if (this.isToolTipDefinedInData && isNullOrUndefined(this.options.content) && isNullOrUndefined(this.options.contentFormatter)) {
						var childNodes = this.contentDiv.getElementsByTagName("span");
						for (var i = 0; i < childNodes.length; i++) {
							if (childNodes[i])
								childNodes[i].style.color = childNodes[i].getAttribute("data-color");
						}
					}
	...
	ToolTip.prototype.getToolTipInnerHTML = function (e) {
		...
		if (this.isToolTipDefinedInData && ((this.content && typeof (this.content) === "function") || this.contentFormatter)) {
			...
			if (this.shared && this.chart.plotInfo.axisPlacement !== "none") {
				...
				for (var i = 0; i < entries.length; i++) {
					...
					if (i === 0 && this.isToolTipDefinedInData && !this.content) {
					...
					if (dataSeries.type === "line" || dataSeries.type === "stepLine" || dataSeries.type === "spline" || dataSeries.type === "area" || dataSeries.type === "stepArea" || dataSeries.type === "splineArea" || dataSeries.type === "column" || dataSeries.type === "bar" || dataSeries.type === "scatter"
						...
						toolTipContent += dataPoint.toolTipContent ? dataPoint.toolTipContent : dataSeries.toolTipContent ? dataSeries.toolTipContent : this.content && typeof (this.content) !== "function" ? this.content : "<span data-color='\"" + (this.options.fontColor ? "" : "'{color}'") + "\"'>{name}:</span>&nbsp;&nbsp;{y}";
						...
						toolTipContent += dataPoint.toolTipContent ? dataPoint.toolTipContent : dataSeries.toolTipContent ? dataSeries.toolTipContent : this.content && typeof (this.content) !== "function" ? this.content : "<span data-color='\"" + (this.options.fontColor ? "" : "'{color}'") + "\"'>{name}:</span>&nbsp;&nbsp;{y}, &nbsp;&nbsp;{z}";
						...
						toolTipContent += dataPoint.toolTipContent ? dataPoint.toolTipContent : dataSeries.toolTipContent ? dataSeries.toolTipContent : this.content && typeof (this.content) !== "function" ? this.content : "<span data-color='\"" + (this.options.fontColor ? "" : "'{color}'") + "\"'>{name}:</span>&nbsp;&nbsp;{y[0]},&nbsp;{y[1]}";
					} else if (dataSeries.type === "candlestick" || dataSeries.type === "ohlc") {
						if (this.chart.axisX && this.chart.axisX.length > 1)
							toolTipContent += (prevDataSeriesXIndex != dataSeries.axisXIndex) ? (dataSeries.axisX.title ? dataSeries.axisX.title + "<br/>" : "X:{axisXIndex}<br/>") : "";

						toolTipContent += dataPoint.toolTipContent ? dataPoint.toolTipContent : dataSeries.toolTipContent ? dataSeries.toolTipContent : this.content && typeof (this.content) !== "function" ? this.content : "<span data-color='\"" + (this.options.fontColor ? "" : "'{color}'") + "\"'>{name}:</span>"
							+ "<br/>Open: &nbsp;&nbsp;{y[0]}"
							+ "<br/>High: &nbsp;&nbsp;&nbsp;{y[1]}"
							+ "<br/>Low:&nbsp;&nbsp;&nbsp;{y[2]}"
							+ "<br/>Close: &nbsp;&nbsp;{y[3]}";
				...
				if (dataSeries.type === "line" || dataSeries.type === "stepLine" || dataSeries.type === "spline" || dataSeries.type === "area" || dataSeries.type === "stepArea" || dataSeries.type === "splineArea" || dataSeries.type === "column" || dataSeries.type === "bar" || dataSeries.type === "scatter"
					|| dataSeries.type === "stackedColumn" || dataSeries.type === "stackedColumn100" || dataSeries.type === "stackedBar" || dataSeries.type === "stackedBar100"
					|| dataSeries.type === "stackedArea" || dataSeries.type === "stackedArea100" || dataSeries.type === "waterfall") {
					toolTipContent = dataPoint.toolTipContent ? dataPoint.toolTipContent : dataSeries.toolTipContent ? dataSeries.toolTipContent : this.content && typeof (this.content) !== "function" ? this.content : "<span data-color='\"" + (this.options.fontColor ? "" : "'{color}'") + "\"'>" + (dataPoint.label ? "{label}" : "{x}") + ":</span>&nbsp;&nbsp;{y}";
				} else if (dataSeries.type === "bubble") {
					toolTipContent = dataPoint.toolTipContent ? dataPoint.toolTipContent : dataSeries.toolTipContent ? dataSeries.toolTipContent : this.content && typeof (this.content) !== "function" ? this.content : "<span data-color='\"" + (this.options.fontColor ? "" : "'{color}'") + "\"'>" + (dataPoint.label ? "{label}" : "{x}") + ":</span>&nbsp;&nbsp;{y}, &nbsp;&nbsp;{z}";
				} else if (dataSeries.type === "pie" || dataSeries.type === "doughnut" || dataSeries.type === "funnel" || dataSeries.type === "pyramid") {
					toolTipContent = dataPoint.toolTipContent ? dataPoint.toolTipContent : dataSeries.toolTipContent ? dataSeries.toolTipContent : this.content && typeof (this.content) !== "function" ? this.content : "<span data-color='\"" + (this.options.fontColor ? "" : "'{color}'") + "\"'>" + (dataPoint.name ? "{name}:</span>&nbsp;&nbsp;" : dataPoint.label ? "{label}:</span>&nbsp;&nbsp;" : "</span>") + "{y}";
				} else if (dataSeries.type === "rangeColumn" || dataSeries.type === "rangeBar" || dataSeries.type === "rangeArea" || dataSeries.type === "rangeSplineArea" || dataSeries.type === "error") {
					toolTipContent = dataPoint.toolTipContent ? dataPoint.toolTipContent : dataSeries.toolTipContent ? dataSeries.toolTipContent : this.content && typeof (this.content) !== "function" ? this.content : "<span data-color='\"" + (this.options.fontColor ? "" : "'{color}'") + "\"'>" + (dataPoint.label ? "{label}" : "{x}") + " :</span>&nbsp;&nbsp;{y[0]}, &nbsp;{y[1]}";
				} else if (dataSeries.type === "candlestick" || dataSeries.type === "ohlc") {
					toolTipContent = dataPoint.toolTipContent ? dataPoint.toolTipContent : dataSeries.toolTipContent ? dataSeries.toolTipContent : this.content && typeof (this.content) !== "function" ? this.content : "<span data-color='\"" + (this.options.fontColor ? "" : "'{color}'") + "\"'>" + (dataPoint.label ? "{label}" : "{x}") + "</span>"
						+ "<br/>Open: &nbsp;&nbsp;{y[0]}"
						+ "<br/>High: &nbsp;&nbsp;&nbsp;{y[1]}"
						+ "<br/>Low: &nbsp;&nbsp;&nbsp;&nbsp;{y[2]}"
						+ "<br/>Close: &nbsp;&nbsp;{y[3]}";
...
CanvasJSChart.version = "v3.6.7 GA";
